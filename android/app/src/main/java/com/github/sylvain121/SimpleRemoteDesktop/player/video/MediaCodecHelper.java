...
public class MediaCodecHelper {
	...
	private static final String TAG = "MEDIA_CODEC_HELPER";
	...
	public static void initializeWithContext(Context context) {
		...
		if (configInfo.reqGlEsVersion != ConfigurationInfo.GL_ES_VERSION_UNDEFINED) {
			Log.d(TAG,"OpenGL ES version: "+configInfo.reqGlEsVersion);

			// Tegra K1 and later can do reference frame invalidation properly
			if (configInfo.reqGlEsVersion >= 0x30000) {
				Log.d(TAG,"Added omx.nvidia to AVC reference frame invalidation support list");
				refFrameInvalidationAvcPrefixes.add("omx.nvidia");

				Log.d(TAG,"Added omx.qcom to AVC reference frame invalidation support list");
				...
				if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.M) {
					Log.d(TAG,"Added omx.intel to AVC reference frame invalidation support list");
		...
		for (String preferredDecoder : preferredDecoders) {
			for (MediaCodecInfo codecInfo : getMediaCodecList()) {
				...
				if (preferredDecoder.equalsIgnoreCase(codecInfo.getName())) {
					Log.d(TAG,"Preferred decoder choice is "+codecInfo.getName());
					return codecInfo;
				}
			}
		}
	...
	public static MediaCodecInfo findFirstDecoder(String mimeType) {
		for (MediaCodecInfo codecInfo : getMediaCodecList()) {
			...
			if (isDecoderInList(blacklistedDecoderPrefixes, codecInfo.getName())) {
				Log.d(TAG,"Skipping blacklisted decoder: "+codecInfo.getName());
				continue;
			}
			...
			for (String mime : codecInfo.getSupportedTypes()) {
				if (mime.equalsIgnoreCase(mimeType)) {
					Log.d(TAG,"First decoder choice is "+codecInfo.getName());
					return codecInfo;
				}
			}
		}
    ...
    private static MediaCodecInfo findKnownSafeDecoder(String mimeType, int requiredProfile) throws Exception {
		for (MediaCodecInfo codecInfo : getMediaCodecList()) {		
			...
			for (String mime : codecInfo.getSupportedTypes()) {
				if (mime.equalsIgnoreCase(mimeType)) {
					Log.d(TAG,"Examining decoder capabilities of "+codecInfo.getName());
					...
					if (requiredProfile != -1) {
						for (CodecProfileLevel profile : caps.profileLevels) {
							if (profile.profile == requiredProfile) {
								Log.d(TAG,"Decoder " + codecInfo.getName() + " supports required profile");
								return codecInfo;
							}
						}

						Log.d(TAG,"Decoder " + codecInfo.getName() + " does NOT support required profile");
					}
					else {
						return codecInfo;
					}
			...
			if (files != null) {
				for (File f : files) {
					if (stringContainsIgnoreCase(f.getName(), "exynos4")) {
						Log.d(TAG,"Found exynos4 in /sys/devices/system");
						return true;
					}
				}
			}
